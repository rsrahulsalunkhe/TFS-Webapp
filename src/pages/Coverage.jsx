...
const Coverage = () => {
  ...
  const loadFutures = async (pageNo) => {
    try {
      const res = await fetchData(`/commoditydetailscoverage/${commodity}?pageno=${pageNo}&pagesize=20`);
        ...
        <div className="bg-primary">
          {futuresList.map((p, index) => {
            ...
            if (index === triggerIndex) {
              return (
                <div className="w-100 h-100" key={index} ref={handleObserver}>
                  <img className="w-100 h-100" src={p.i} alt="" />
                </div>
              );
            } else {
              return <div className="w-100 h-100" key={index} ref={handleObserver}>
                <img className="w-100 h-100" src={p.i} alt="" />
              </div>;
            }
          })}
